...
// the link to the article is posted as the first comment (keeping the post body
// link-free, which reads better and avoids LinkedIn's reach penalty on in-body
// outbound links), and the post carries either:
//   - a multi-image carousel, when the post bundle has 2+ `images/slide-N.png`
//     files (a LinkedIn MultiImage post — this needs only the w_member_social
//     scope already granted, not the partner-gated Community Management API), or
//   - the post's single featured image, otherwise (the previous behaviour).
//
...
// Find a post's carousel slide PNGs (images/slide-1.png, slide-2.png, ...),
// generated by the cover-image workflow from the author's slide-N.svg files.
// Returns them in numeric slide order, or [] if the bundle has none.
function resolveSlideImages(postPath) {
  const imagesDir = join(dirname(postPath), "images");
...
// `content` is either { media: { id, altText? } } for a single image, or
// { multiImage: { images: [{ id, altText? }, ...] } } for a carousel (2-20
// images) — or null/undefined to post text-only.
async function postToLinkedIn(commentary, content) {
  const body = {
    author: AUTHOR,
    commentary,
    visibility: "PUBLIC",
  ...
  if (content?.multiImage?.images?.length) {
    body.content = { multiImage: { images: content.multiImage.images } };
  } else if (content?.media?.id) {
    body.content = {
      media: { id: content.media.id, ...(content.media.altText ? { altText: content.media.altText } : {}) },
    };
  }
...
async function main() {
  ...
  for (const { path, fm } of toAnnounce) {
    ...
    const slidePaths = POST_IMAGE ? resolveSlideImages(path) : [];
    const usesCarousel = slidePaths.length >= 2;
    const imagePath = POST_IMAGE && !usesCarousel ? resolveRasterImage(path, fm.image) : null;

    console.log("\n--- Post ---\n" + commentary + "\n------------");
    if (usesCarousel) {
      console.log(`Carousel: ${slidePaths.length} slides (${slidePaths.map((p) => basename(p)).join(", ")})`);
    } else if (POST_IMAGE) {
      console.log(`Image: ${imagePath || "(none resolved — will post text-only)"}`);
    }
    ...
    try {
      ...
      if (usesCarousel) {
        ...
        for (const p of slidePaths) {
          const urn = await uploadImage(p, AUTHOR);
          if (!urn) continue;
          const alt = slideAltText(p);
          images.push({ id: urn, ...(alt ? { altText: alt } : {}) });
        }
        if (images.length >= 2) {
          content = { multiImage: { images } };
        } else {
          console.warn(
            `Only ${images.length}/${slidePaths.length} slide uploads succeeded (LinkedIn needs 2+ for a ` +
              "carousel); falling back to the cover image.",
          );
          const fallbackPath = resolveRasterImage(path, fm.image);
          const imageUrn = fallbackPath ? await uploadImage(fallbackPath, AUTHOR) : null;
          content = imageUrn ? { media: { id: imageUrn, altText: fm.title } } : null;
        }
      } else if (imagePath) {
        const imageUrn = await uploadImage(imagePath, AUTHOR);
        content = imageUrn ? { media: { id: imageUrn, altText: fm.title } } : null;
      }
      const commentaryToPost = LINK_IN_COMMENT ? commentary : `${commentary}\n\nRead more: ${url}`;
      const postUrn = await postToLinkedIn(commentaryToPost, content);
