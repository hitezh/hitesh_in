...
- Cover images are usually authored as an SVG (`images/cover.svg`). Start from [`.github/templates/blog-cover.svg`](.github/templates/blog-cover.svg), then make it specific to the post. The template is styled as a screenshotted X post (avatar, name, handle, an oversized claim as the post text, a hashtag, one attached visual, and an engagement bar) because that format reads well and gets clicked in a fast-moving feed. Do not turn it into a miniature article, use a stock-photo collage, or add more than one diagram. Keep the claim to 2-4 lines, make the hashtag useful, and leave enough empty space for it to be legible at thumbnail size. Engagement-bar counts are cosmetic — keep them plausible, not absurd. Change the template's `aria-label` to describe the finished image.
...
- [ ] Images exist in the page bundle, render correctly, have useful alt text, and include credit where needed. An SVG cover starts from the X-post-style template, has a short readable claim, a specific subhead, and one simple visual. Commit only `cover.svg`; CI generates the `cover.png` social raster (run `node .github/scripts/rasterize-cover.mjs` to preview it locally).
